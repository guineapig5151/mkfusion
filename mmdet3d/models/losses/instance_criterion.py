...
class InstanceCriterion:
    ...
    def get_layer_loss(self, aux_outputs, insts, indices=None):
        ...
        for mask, score, inst, (idx_q, idx_gt) in zip(pred_masks, pred_scores,
            ...
            if len(pred_mask) > 0:
                ...
                mask_bce_losses.append(torch.tensor(0.0, dtype=torch.float32, device=pred_mask.device))
                mask_dice_losses.append(torch.tensor(0.0, dtype=torch.float32, device=pred_mask.device))
    ...
    def __call__(self, pred, insts):
        ...
        for i in range(len(insts)):
            pred_instances = InstanceData_(
                scores=cls_preds[i], # num_query, 
                masks=pred_masks[i]) # num_query, num_query
            gt_instances = InstanceData_(
                labels=insts[i].labels_3d,
                masks=insts[i].sp_masks) # num_gt_ins, num_query
            if insts[i].get('query_masks') is not None:
                gt_instances.query_masks = insts[i].query_masks

            match_cur = self.matcher(pred_instances, gt_instances) # idx_q, idx_gt
            indices.append(match_cur)
        ...
        for cls_pred, inst, (idx_q, idx_gt) in zip(cls_preds, insts, indices):
            ...
            cls_target = cls_pred.new_full(
                (len(cls_pred),), n_classes, dtype=torch.long)
            cls_target[idx_q] = inst.labels_3d[idx_gt] # idx_q---idx_gt
            cls_losses.append(F.cross_entropy(
                cls_pred, cls_target, cls_pred.new_tensor(self.class_weight)))
        cls_loss = torch.mean(torch.stack(cls_losses))
        ...
        for mask, score, inst, (idx_q, idx_gt) in zip(pred_masks, pred_scores,
            ...
            if len(inst) == 0:
                continue
            pred_mask = mask[idx_q] # n_gt, n_query
            tgt_mask = inst.sp_masks[idx_gt]

            # bug pred_mask empty
            if len(pred_mask) > 0:
                mask_bce_losses_cur_batch = F.binary_cross_entropy_with_logits(
                    pred_mask, tgt_mask.float())
                mask_bce_losses.append(mask_bce_losses_cur_batch)                
                mask_dice_losses.append(dice_loss(pred_mask, tgt_mask.float()))
            else:
                mask_bce_losses.append(torch.tensor(0.0, dtype=torch.float32, device=pred_mask.device))
                mask_dice_losses.append(torch.tensor(0.0, dtype=torch.float32, device=pred_mask.device))
        ...
        if 'aux_outputs' in pred:
            ...
            for i, aux_outputs in enumerate(pred['aux_outputs']):
                layer_loss = self.get_layer_loss(aux_outputs, insts, indices)
                assert torch.isnan(layer_loss) == False
                loss += layer_loss
...
class SparseMatcher:
    ...
    def __call__(self, pred_instances, gt_instances, **kwargs):
        ...
        cost_values = []
        for cost in self.costs:
            cost_values.append(cost(pred_instances, gt_instances)) # n_q, num_gt
        # of shape (n_queries, n_gts)
        cost_value = torch.stack(cost_values).sum(dim=0)

        self.inf = torch.tensor(float('inf'), dtype=torch.float32, device=cost_value.device)
        cost_value = torch.where(
            gt_instances.query_masks.T, cost_value.float(), self.inf)

        values = torch.topk(
            cost_value, self.topk + 1, dim=0, sorted=True,
            largest=False).values[-1:, :]
        # ids = torch.argwhere(cost_value < values)
        ids = torch.nonzero(cost_value < values, as_tuple=False)

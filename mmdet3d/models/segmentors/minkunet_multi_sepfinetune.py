...
class MinkUNet_multi_sepfinetune(EncoderDecoder3D):
    def __init__(self,
                 freeze_img = True,
                 use_depth_map = False,
    ...
    def _iter_sep_head_modules(self):
        """返回 sep_heads 所有子模块的扁平迭代器"""
        heads = getattr(self.decode_head, 'sep_heads', None)
        if heads is None:
            return []
        if isinstance(heads, (nn.ModuleList, nn.Sequential, list, tuple)):
            it = []
            for m in heads:
                it += list(m.modules())
            return it
        if isinstance(heads, nn.ModuleDict):
            ...
            return it
        # 单一模块
        return list(heads.modules())

    def _sep_param_ids(self):
        """收集 sep_heads 的 parameter id 集合"""
        ids = set()
        for m in self._iter_sep_head_modules():
    ...
    def freeze_all_but_sep_heads(self, verbose=True):
        """冻结除 sep_heads 外的所有参数，并设置BN等为eval"""
        # 1) 只保留 sep_heads 参数可训练
        sep_param_ids = self._sep_param_ids()
        num_trainable = 0
        num_frozen = 0
        for name, p in self.named_parameters():
            if id(p) in sep_param_ids:
        ...
        # 2) 全模型先 eval()，避免非 sep 的 BN 更新
        super().train(False)

        # 3) 仅 sep_heads 设为 train()，允许它们的 BN/Dropout 正常工作
        for m in self._iter_sep_head_modules():
            m.train(True)
        ...
        if verbose:
            try:
                ...
                def print_log(msg, *args, **kwargs): print(msg)
            print_log(f'[freeze] frozen params: {num_frozen:,d}, trainable(sep_heads): {num_trainable:,d}', 'current')

    def init_weights(self):
        """Initialize model weights and freeze non-sep modules."""
        super(MinkUNet_multi_sepfinetune, self).init_weights()

        # 你原来的图像分支冻结逻辑（若有）
        if getattr(self, 'freeze_img', False):
            if getattr(self, 'with_img_backbone', False):
                print_log('freezed image backbone', 'current')
                for p in self.img_backbone.parameters():
                    p.requires_grad = False
            if getattr(self, 'with_img_neck', False):
                print_log('freezed image neck', 'current')
                for p in self.img_neck.parameters():
                    p.requires_grad = False

        # 关键：冻结除 sep_heads 外的参数，并锁住 BN 更新
        self.freeze_all_but_sep_heads(verbose=True)

    # --- 重要 ---
    # 覆盖 train()：防止外部调用 model.train() 把冻结模块误设为train
    def train(self, mode: bool = True):
        # 全部设为 eval（BN不更新）
        super().train(False)
        # 仅 sep_heads 按 mode 切换
        for m in self._iter_sep_head_modules():
            m.train(mode)
        return self
    ...
    def extract_feat(self, batch_inputs_dict: dict,
        ...
        pts_feats = self.extract_voxel_feat(
            batch_inputs_dict,
            img_feats=img_feats,
    ...
    def loss(self, inputs: dict, data_samples):
        ...
        x = self.extract_feat(inputs, batch_input_metas)

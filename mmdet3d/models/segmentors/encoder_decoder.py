...
class EncoderDecoder3D(Base3DSegmentor):
    ...
    def loss(self, batch_inputs_dict: dict,
        ...
        if self.outdoor:
            # points = self.padding_concat(batch_inputs_dict['points'])
            x = []
            for points in batch_inputs_dict['points']:
                x.append(self.extract_feat(points.unsqueeze(0)))
        else:
        ...
        if self.outdoor:
            loss_decode = defaultdict(float)
            for i, _x in enumerate(x):
                loss_decode_single = self._decode_head_forward_train(_x, [batch_data_samples[i]])
                for k, v in loss_decode_single.items():
                    loss_decode[k] += v
    ...
    def predict(self,
        ...
        return self.postprocess_result(seg_logits_list, batch_data_samples, points=points)

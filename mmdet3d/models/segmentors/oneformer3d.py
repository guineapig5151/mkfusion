...
class ScanNetOneFormer3DMixin:
    ...
    def predict_by_feat(self, out, superpoints, just_seg):
    ...
    def predict_by_feat_instance(self, out, superpoints, score_threshold):
        ...
        """
        cls_preds = out['cls_preds'][0] # num_query num_ins 19
        pred_masks = out['masks'][0] # num_query, num_query

        scores = F.softmax(cls_preds, dim=-1)[:, :-1] # num_query 18
    ...
    def predict_by_feat_semantic(self, out, superpoints, classes=None):
        ...
        if classes is None:
            ...
            classes = list(range(out['sem_preds'][0].shape[1]))
...
class ScanNetOneFormer3D(ScanNetOneFormer3DMixin, Base3DDetector):
    ...
    def extract_feat(self, x, superpoints, inverse_mapping, batch_offsets):
        ...
        batch_mask = x.indices[:,0]
        is_strictly_increasing = torch.all(batch_mask[1:] >= batch_mask[:-1])
    ...
    def loss(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        for i in range(len(batch_data_samples)):
            ...
            assert gt_pts_seg.sp_pts_mask.shape[-1] == batch_inputs_dict['points'][i].shape[0]
            sp_pts_masks.append(gt_pts_seg.sp_pts_mask.squeeze(0))
    ...
    def predict(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        results_list = self.predict_by_feat(x, sp_pts_masks, self.just_seg)

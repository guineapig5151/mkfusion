...
class MinkUNet_multi(EncoderDecoder3D):
    def __init__(self,
                 freeze_img = True,
                 use_depth_map = False,
        ...
        if self.use_depth_map:
            ...
            for i in range(len(self.input_cam_dim)):
                depth_conv = nn.Sequential(
                    nn.Conv2d(input_depth_channel, out_depth_channel, kernel_size=3, stride=1, padding=1),
                    nn.GroupNorm(num_groups=8, num_channels=out_depth_channel),
    ...
    def init_weights(self):
        ...
        super(MinkUNet_multi, self).init_weights()

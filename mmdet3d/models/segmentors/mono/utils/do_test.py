...
def get_prediction(
    ...
    pred_depth, confidence, output_dict, dict_out = model.module.inference(data)
    ...
    pred_depth = pred_depth * normalize_scale / scale_info # shape =torch.Size([1216, 1936])
    if gt_depth is not None:
    ...
    merged_dict = {**output_dict, **dict_out}

    return pred_depth, pred_depth_scale, scale, merged_dict
...
def do_scalecano_test_with_custom_data(
    ...
    test_data = sorted(test_data, key=lambda x: x['rgb'][-9:])
    ...
    for i, an in enumerate(test_data):
        ...
        rgb_test = an['cur_batch_img'].unsqueeze(0)
        ...
        return_dict['pred_depth'] = pred_depth
        return_dict['output'] = output
                
        save_raw_depth = False
        if save_raw_depth:
            pred_depth_save = (pred_depth.detach().cpu().numpy() * 256).astype(np.uint16)
            pred_depth_save = Image.fromarray(pred_depth_save)
            save_path = '_z_vis/'
            pred_depth_save.save(osp.join(save_path, an['filename'][-9:-4]+'.png'))
    ...
    if save_normal:
        ...
        pred_normal_save: np.ndarray = pred_normal[:, :, pad[0]:H-pad[1], pad[2]:W-pad[3]].squeeze().cpu().numpy().astype(np.float32)
        save_normal_dir = osp.join(save_pcd_dir, '..', 'normal')
        os.makedirs(save_normal_dir, exist_ok=True)
        pred_normal_save.tofile(osp.join(save_normal_dir, an['filename'][:-4]+'.npy'))


    if i % save_interval == 0 and vis:
        ...
        lidar_pts = np.fromfile(os.path.join('/dataset/view_of_delft_PUBLIC/lidar/training/velodyne', an['filename'][:-4]+'.bin'), dtype=np.float32).reshape((-1, 4))
        calib_path = os.path.join('/dataset/view_of_delft_PUBLIC/lidar/training/calib', an['filename'][:-4]+'.txt')
        with open(calib_path, 'r') as f:
            lines = f.readlines()
        Tv2c = np.array([float(x) for x in lines[5].strip().split(' ')[1:]], dtype=np.float32).reshape((3, 4))
        temp = np.zeros((4, 4), dtype=np.float32)
        temp[:3, :4] = Tv2c
        temp[-1, -1] = 1
        Tv2c = temp
        lidar_pts[:, -1] = 1
        lidar_pts = lidar_pts @ Tv2c.T
        pseudo_pts = pcd.reshape((-1, 3)).astype(np.float32)
        pseudo_rgb = rgb_origin.reshape(-1, 3).astype(np.float32)
        pseudo_pts = np.concatenate([pseudo_pts, pseudo_rgb], axis=-1)
        lidar_rgb = np.zeros((lidar_pts.shape[0], 3), dtype=np.float32)
        lidar_rgb[:, 0] = 255.0
        lidar_pts = np.concatenate([lidar_pts[:, :3], lidar_rgb], axis=-1)
        all_pts = np.concatenate([lidar_pts, pseudo_pts], axis=0)
        all_pts.tofile(osp.join(save_pcd_dir, an['folder'], an['filename'][:-4]+'.bin'))

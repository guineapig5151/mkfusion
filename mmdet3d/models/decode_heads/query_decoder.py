...
class CrossAttentionLayer(BaseModule):
    ...
    def forward(self, sources, queries, attn_masks=None):
        ...
        for i in range(len(sources)):
            ...
            # yyt_c batch?
            output, _ = self.attn(queries[i].unsqueeze(0), k.unsqueeze(0), v.unsqueeze(0), attn_mask=attn_mask)
...
class SelfAttentionLayer(BaseModule):
    ...
    def forward(self, x):
        ...
        out = []
        for y in x:
            z, _ = self.attn(y.unsqueeze(0), y.unsqueeze(0), y.unsqueeze(0))
            z = z.squeeze(0)
            z = self.dropout(z) + y
            z = self.norm(z)
            out.append(z)
...
class ScanNetQueryDecoder(QueryDecoder):
    ...
    def forward_iter_pred(self, x, queries):
        ...
        inst_feats = [self.input_proj(y) for y in x]
        mask_feats = [self.x_mask(y) for y in x]
        queries = self._get_queries(queries, len(x)) # query proj+emb
        cls_pred, sem_pred, pred_score, pred_mask, attn_mask = \
            self._forward_head(queries, mask_feats, last_flag=False) # vod
        cls_preds.append(cls_pred)

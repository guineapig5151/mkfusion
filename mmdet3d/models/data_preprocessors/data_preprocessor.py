...
class Det3DDataPreprocessor(DetDataPreprocessor):
    ...
    def voxelize(self, points: List[Tensor],
        ...
        point_cloud_range = points[0].new_tensor(self.voxel_layer.point_cloud_range)
        ...
        if self.voxel_type == 'hard':
            ...
            if self.name is not None:
                voxels_sweep = []
                coors_sweep = []
                for batch_id, data_sample in enumerate(zip(points, data_samples)):
                    if self.name == 'sweep_point_list':
                        sweep_points = data_samples[batch_id].sweep_point_list.tensor.cuda()
                        # 不足补全
                        if sweep_points.shape[0] == 0:
                            sweep_points = points[batch_id]
                    elif self.name == 'lidar_point':
                        sweep_points = data_samples[batch_id].lidar_point.tensor.cuda()
                        # 不足补全
                        if sweep_points.shape[0] == 0:
                            sweep_points = points[batch_id]
                    res_coors = torch.floor((sweep_points[:, :3] - point_cloud_range[0:3]) / voxel_size).int()
                    ...
                    res_voxels = sweep_points[inds]
                    if self.batch_first:
                        res_voxel_coors = F.pad(
                            res_voxel_coors, (1, 0), mode='constant', value=batch_id)
                    else:
                        res_voxel_coors = F.pad(
                            res_voxel_coors, (0, 1), mode='constant', value=batch_id)    
                    voxels_sweep.append(res_voxels)
                    coors_sweep.append(res_voxel_coors)
                voxels_sweep = torch.cat(voxels_sweep, dim=0)
                coors_sweep = torch.cat(coors_sweep, dim=0)
                voxel_dict['voxelssweep_point_list'] = voxels_sweep
                voxel_dict['coorssweep_point_list'] = coors_sweep 

            voxels, coors = [], []
            voxel_size = points[0].new_tensor(self.voxel_layer.voxel_size)
            
            for i, (res, data_sample) in enumerate(zip(points, data_samples)):
                res_coors = torch.floor((res[:, :3] - point_cloud_range[0:3]) / voxel_size).int()
                res_coors_numpy = res_coors.cpu().numpy()
                inds, point2voxel_map = self.sparse_quantize(
                    res_coors_numpy, return_index=True, return_inverse=True)
        ...
        elif self.voxel_type == 'minkunet_multi':
            voxels, coors = [], []
            for i, res in enumerate(points):
                
                if torch.isnan(res).any():
                    print(f'[NaN] in input res[{i}]')

                # 检查 voxel 坐标计算是否出错
                tmp_coords = (res[:, :3] - point_cloud_range[0:3]) / voxel_size
                if torch.isnan(tmp_coords).any():
                    print(f'[NaN] in voxel coordinate transform, sample {i}')
                ...
                if len(inds) == 0:
                    print(f'[WARN] Empty inds after sparse_quantize at sample {i}')
                ...
                if torch.isnan(res_voxels).any():
                    print(f'[NaN] in res_voxels[{i}] after indexing')
                if torch.isnan(res_voxel_coors).any():
                    print(f'[NaN] in res_voxel_coors[{i}] after indexing') 

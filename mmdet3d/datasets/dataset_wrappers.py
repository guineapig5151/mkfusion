...
class CBGSDataset:
    ...
    def _get_sample_indices(self, dataset: BaseDataset) -> List[int]:
        ...
        """
        # 先计算每个类别所存在的总帧数，然后计算每个类别占总帧数的比例，然后对每个类别所在的帧数进行随机抽帧，保证每个类别的抽帧数一致，也就是抽帧后每个类别所在的帧数是相同的，保证了样本的均衡性
        ...
        for idx in range(len(dataset)):
            ...
            for cat_id in sample_cat_ids:
                if cat_id != -1 and cat_id != self.ignore_id:
        ...
        epsilon = 1e-10
        ratios = [frac / (v + epsilon) for v in class_distribution.values()]
        for cls_inds, ratio in zip(list(class_sample_idxs.values()), ratios):
            if len(cls_inds) > 0: # yyt_c for background class

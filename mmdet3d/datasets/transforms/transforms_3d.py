...
class AddSuperPointAnnotations(BaseTransform):
    ...
    def transform(self, input_dict):
        ...
        num_fore_ins = torch.sum(pts_instance_mask != -1)
        pts_semantic_mask = torch.tensor(input_dict['pts_semantic_mask'])
        num_fore_seg = torch.sum(pts_semantic_mask != 9)
        # assert num_fore_ins == num_fore_seg 
        ...
        if 'sp_pts_mask' not in input_dict:
            # assert pts_instance_mask.size(0) == input_dict['points'].shape[0]
            sp_pts_mask = torch.arange(pts_instance_mask.size(0))
            input_dict['sp_pts_mask'] = sp_pts_mask.unsqueeze(0)
            sp_masks_inst_thre = 0.5
            sp_masks_seg_thre = 0.5
        else:
            sp_masks_inst_thre = 0.5
            sp_masks_seg_thre = 0.5
        ...
        if insts_mask.shape[1] != 0:
            ...
            sp_masks_inst = sp_masks_inst > sp_masks_inst_thre
            num_fore_ins_mask = torch.sum(sp_masks_inst > 0)
            # assert num_fore_ins_mask == num_fore_ins
        ...
        input_dict['gt_labels_3d'] = gt_labels.numpy() # gt对应的类别
        ...
        sp_masks_seg = sp_masks_seg > sp_masks_seg_thre
        sp_masks_seg[-1, sp_masks_seg[:-1,:].sum(axis=0) == 0] = True # background 类别
        num_fore_seg_mask = torch.sum(sp_masks_seg[:-1, :] > 0)
        # assert num_fore_seg_mask == num_fore_ins        
...
class RandomFlip3D(RandomFlip):
            ...
            if 'gt_bboxes_3d' in input_dict:
                ...
                if name_list is not None:
                    for name in name_list:
                        input_dict[name] = input_dict['gt_bboxes_3d'].flip(
                        direction, points=input_dict[name])
                else:
                ...
                if name_list is not None:
                    for name in name_list:
                        input_dict[name] = input_dict[name].flip(direction)
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        if input_dict['pcd_horizontal_flip']:
            self.random_flip_data_3d(input_dict, 'horizontal', self.name_list)
            input_dict['transformation_3d_flow'].extend(['HF'])

        if input_dict['pcd_vertical_flip']:
            self.random_flip_data_3d(input_dict, 'vertical', self.name_list)
    ...
    def _rot_bbox_points(self, input_dict: dict) -> None:
        ...
        if 'gt_bboxes_3d' in input_dict and \
            ...
            for name in self.name_list:
                input_dict[name], rot_mat_T = input_dict['gt_bboxes_3d'].rotate(
                noise_rotation, input_dict[name])
        else:
...
class PointsRangeFilter(BaseTransform):
    ...
    def transform(self, input_dict: dict) -> dict:
        points = input_dict['points']
        if self.use_teacher:
            points_student = points[points.tensor[:,-1] == -66]
            points_teacher = points[points.tensor[:,-1] == -88]
            points_student_mask = points_student.in_range_3d(self.pcd_range)
            clean_points_student = points_student[points_student_mask]
            points_teacher_mask = points_teacher.in_range_3d(self.pcd_range)
            clean_points_teacher = points_teacher[points_teacher_mask]
            clean_points = clean_points_student.cat([clean_points_student, clean_points_teacher])
            input_dict['points'] = clean_points
            if self.test_teacher == True:
                points_mask = points_teacher_mask.numpy()
            else:
                points_mask = points_student_mask.numpy()
        else:
            ...
            if len(self.name_list):
                for name in self.name_list:
                    points_sweep = input_dict[name]
                    points_mask_sweep = points_sweep.in_range_3d(self.pcd_range)
                    clean_points_sweep = points_sweep[points_mask_sweep]
                    input_dict[name] = clean_points_sweep
                   
            points_mask = points_mask.numpy()

        if input_dict['points'].shape[0] == 0:
        ...
        if pts_semantic_mask is not None:
            ...
            if 'eval_ann_info' in input_dict:
                input_dict['eval_ann_info']['pts_semantic_mask'] = pts_semantic_mask[points_mask]
        return input_dict

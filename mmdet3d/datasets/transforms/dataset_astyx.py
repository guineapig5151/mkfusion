...
class Calibration(object):
    ...
    def __init__(self, filepath: str) -> None:
        ...
        global DATA_ROOT
        with open(DATA_ROOT + filepath, "r") as file_handler:
    ...
    def get_lidar_to_camera(self):
        return np.dot(Calibration._inv(self.camera), self.lidar)

    def get_lidar2img(self):
        lidar_to_cam = self.get_lidar_to_camera()
        return np.dot(self.cameraCoef, lidar_to_cam[:3, :])  # 3x4 投影矩阵
    ...
    def to_dict(self) -> dict:
        """Convert calibration matrices to serializable dict."""
        calibration_dict = {}

        if hasattr(self, 'camera'):
            calibration_dict['camera'] = self.camera.tolist()
        if hasattr(self, 'cameraCoef'):
            calibration_dict['cameraCoef'] = self.cameraCoef.tolist()
        if hasattr(self, 'lidar'):
            calibration_dict['lidar'] = self.lidar.tolist()
        if hasattr(self, 'radar'):
            calibration_dict['radar'] = self.radar.tolist()
...
class Camera(object):
    ...
    def __init__(self, filepath: str, calibration: np.array,
        ...
        """
        global DATA_ROOT
        self.filepath: str = DATA_ROOT + filepath
...
class AstyxDataset(object):
    ...
    def __init__(self,
                 dataset = 'dataset_astyx_hires2019/dataset.json',
                 ) -> None:
        ...
        self.dataset: str = DATA_ROOT + dataset

...
class Seg3DDataset(BaseDataset):
    ...
    def parse_data_info(self, info: dict, default_cam_key=None) -> dict:
        ...
        if self.modality['use_camera']:
            ...
            self.default_cam_key = default_cam_key # vod
    ...
    def prepare_data(self, idx: int) -> dict:
        ...
        if self.use_tem:
            sweep_num = len(data_info['sweeps'])
            num_tem = min(self.num_tem, sweep_num)
            data_info_before_list = []
            for before_tem_id in range(1, num_tem):
                data_info_before = self.get_data_info(idx - before_tem_id)
                data_info_before_list.append(data_info_before)
            data_info['data_info_before_list'] = data_info_before_list

        data_info['dataset'] = self
        
        example =  self.pipeline(data_info)
        return example
        # else:

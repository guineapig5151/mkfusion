...
default_hooks = dict(
    checkpoint=dict(
        interval=1, rule='greater', save_best='miou', type='CheckpointHook'),
...
eval_pipeline = [
    ...
    dict(
        depth_map_path='data/vod_depth_metric3dv2/',
        type='Loaddepthmap'),
    dict(
        dataset_type='vod',
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys=dict({
            ...
            'backbone.fusion_blocks': dict(lr_mult=1.0),
            'decode_head': dict(lr_mult=0.1),
            'depth_convs': dict(lr_mult=1.0),

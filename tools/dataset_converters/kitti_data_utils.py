...
def get_kitti_image_info(path,
    ...
    def map_func(idx):
        ...
        if with_pose:
            jsons = []
            pose_file = str(root_path) + '/training' + '/pose/' + f"{idx:05}.json"
            ...
            t_utm_camera = np.eye(4)
            pose = {
                't_odom_camera': t_odom_camera,
                't_map_camera': t_map_camera,
                't_utm_camera': t_utm_camera
            }
            info['pose'] = pose

        if with_clips:
            clip_path = str(root_path) + '/clips/'
            clip_name_list = os.listdir(clip_path)
            clip_name_list.sort()
            clip_name_list.pop(-1)
            for i in range(len(clip_name_list)):
                with open(clip_path + clip_name_list[i]) as f:
                    cur_data_list = f.readlines()
                    cur_data_list = [int(line.strip()) for line in cur_data_list]
                if idx in cur_data_list:
                    info['clip_index'] = clip_name_list[i][:-4]
                    max_before_sweeps_num = cur_data_list.index(idx)
                    break
            
        if with_sweep:
            sweeps = []
            max_sweeps = 12
            for i in range(min(max_sweeps, max_before_sweeps_num)):
                sweeps_cur = {}
                idx_before = idx - i
                sweeps_cur['timestamp'] = i
                sweeps_cur['velodyne_path'] = get_velodyne_path(
                idx_before, path, training, relative_path)

                if calib:
                    calib_path_before = get_calib_path(
                    idx_before, path, training, relative_path=False)
                    with open(calib_path_before, 'r') as f:
                        lines = f.readlines()
                    P0 = np.array([float(info) for info in lines[0].split(' ')[1:13]
                                    ]).reshape([3, 4])
                    P1 = np.array([float(info) for info in lines[1].split(' ')[1:13]
                    ...
                    if extend_matrix:
                        ...
                        # Tr_imu_to_velo = _extend_matrix(Tr_imu_to_velo)
                    calib_info_before = {}
                    calib_info_before['P0'] = P0
                    calib_info_before['P1'] = P1
                    calib_info_before['P2'] = P2
                    calib_info_before['P3'] = P3
                    calib_info_before['R0_rect'] = rect_4x4
                    calib_info_before['Tr_velo_to_cam'] = Tr_velo_to_cam
                    # calib_info_before['Tr_imu_to_velo'] = Tr_imu_to_velo
                    sweeps_cur['calib'] = calib_info_before
                    jsons = []
                    pose_file = str(root_path) + '/training' + '/pose/' + f"{idx_before:05}.json"
                    for line in open(pose_file, 'r'):
                        jsons.append(json.loads(line))
                    t_odom_camera = np.array(jsons[0]["odomToCamera"], dtype=np.float32).reshape(4, 4)
                    #t_map_camera = np.array(jsons[1]["mapToCamera"], dtype=np.float32).reshape(4, 4)
                    ...
                    sweeps_cur['pose'] = pose

                # static static_compensation
                t_odom_cam_1 = pose_before['t_odom_camera']
                t_odom_cam_2 = pose['t_odom_camera']
                t_cam_radar_1 = calib_info_before['Tr_velo_to_cam']
                t_cam_radar_2 = calib_info['Tr_velo_to_cam']
                odom_radar_1 = np.dot(t_odom_cam_1, t_cam_radar_1)
                odom_radar_2 = np.dot(t_odom_cam_2,t_cam_radar_2)
                radar2_radar1 = np.dot(np.linalg.inv(odom_radar_2), odom_radar_1)
                sweeps_cur['radar2_radar1'] = radar2_radar1
                sweeps.append(sweeps_cur)
            info['sweeps'] = sweeps
        return info
...
def get_dualradar_image_info(path,
                         training=True,
                         label_info=True,
                         velodyne=False,
                         calib=False,
    ...
    def map_func(idx):
        info = {}
        # ars548: ['x', 'y', 'z', 'intensity','dopple']
        # arbe: ['x', 'y', 'z', 'intensity']
        # lidar: ['x', 'y', 'z', 'intensity', 'ring', 'timestamp']
        if radar_type == 'arbe':
            pc_info = {'num_features': 4}
        elif radar_type == 'ars548':
            pc_info = {'num_features': 5}
        elif radar_type == 'robosense':
        ...
        if velodyne:
            ...
            pc_info['velodyne_path'] = get_kitti_info_path(idx, path, radar_type, '.bin', training,
                               relative_path, exist_check=True, use_prefix_id=True)
        image_info['image_path'] = get_image_path(idx, path, training,
                                                  relative_path, info_type='image', file_tail='.png', use_prefix_id=True)
        if with_imageshape:
            img_path = image_info['image_path']
            if relative_path:
                img_path = str(root_path / img_path)
            image_info['image_shape'] = np.array(
                io.imread(img_path).shape[:2], dtype=np.int32)
        if label_info:
            label_path = get_label_path(idx, path, training, relative_path, info_type='label', use_prefix_id=True)
            if relative_path:
                label_path = str(root_path / label_path)
            annotations = get_label_anno(label_path)
        info['image'] = image_info
        ...
        if calib:
            ...
            if extend_matrix:
                Tr_velo_to_cam = _extend_matrix(Tr_velo_to_cam)
                Tr_radar_to_velo = _extend_matrix(Tr_radar_to_velo)
                # Tr_imu_to_velo = _extend_matrix(Tr_imu_to_velo)
            calib_info['P1'] = P2
            calib_info['P2'] = P2
            calib_info['P3'] = P2
            ...
            calib_info['R0_rect'] = rect_4x4
            calib_info['Tr_velo_to_cam'] = Tr_velo_to_cam
            calib_info['Tr_radar_to_velo'] = Tr_radar_to_velo
            # calib_info['Tr_imu_to_velo'] = Tr_imu_to_velo
            info['calib'] = calib_info
        ...
        assert with_pose == False
        assert with_clips == False
        assert with_sweep == False
        return info

...
def update_kitti_infos(pkl_path, out_dir):
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        temp_data_info['sweeps'] = ori_info_dict['sweeps']
...
def update_dualradar_infos(pkl_path, out_dir):
    # 2025/05/17 lhs: 和update_kitti_infos没差别，除了METAINFO不一样
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
...
def update_pkl_infos(dataset, out_dir, pkl_path):
    ...
    elif dataset.lower() == 'dualradar':
        update_dualradar_infos(pkl_path=pkl_path, out_dir=out_dir)

...
from .kitti_data_utils import WaymoInfoGatherer, get_kitti_image_info, get_dualradar_image_info
...
def _calculate_num_points_in_gt_dualradar(data_path,
                                infos,
                                relative_path,
                                remove_outside=True,
                                num_features=7,
    ...
    for info in mmengine.track_iter_progress(infos):
        ...
        points_hom = np.concatenate((points_v[:,0:3],np.ones((points_v.shape[0],1))), axis=-1)
        point_lidar = points_hom @ info['Tr_radar_to_velo'].T
        points_v[:,0:3] = point_lidar
        ...
        gt_boxes_camera = np.concatenate([loc, dims, rots[..., np.newaxis]],
                                         axis=1)
        gt_boxes_lidar = box_np_ops.box_camera_to_lidar_dualradar(
            gt_boxes_camera, rect, Trv2c)
        indices = box_np_ops.points_in_rbbox(points_v[:, :3], gt_boxes_lidar)
        num_points_in_gt = indices.sum(0)
        num_ignored = len(annos['dimensions']) - num_obj
...
def create_dualradar_info_file(data_path,
                           pkl_prefix='kitti',
                           with_plane=False,
                           save_path=None,
                           relative_path=True,
    ...
    if save_path is None:
        ...
        save_path = Path(save_path)
    kitti_infos_train = get_dualradar_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    num_features = kitti_infos_train[0]['point_cloud']['num_features']
    _calculate_num_points_in_gt_dualradar(data_path, kitti_infos_train, relative_path, num_features=num_features, radar_type=radar_type)
    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'Kitti info train file is saved to {filename}')
    mmengine.dump(kitti_infos_train, filename)
    kitti_infos_val = get_dualradar_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
        ...
        with_pose=False,
        radar_type=radar_type)
    _calculate_num_points_in_gt_dualradar(data_path, kitti_infos_val, relative_path, num_features=num_features, radar_type=radar_type)
    filename = save_path / f'{pkl_prefix}_infos_val.pkl'
    print(f'DualRadar info val file is saved to {filename}')
    mmengine.dump(kitti_infos_val, filename)
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'DualRadar info trainval file is saved to {filename}')
    mmengine.dump(kitti_infos_train + kitti_infos_val, filename)

    kitti_infos_test = get_dualradar_image_info(
        data_path,
        training=False,
        label_info=False,
        velodyne=True,
        ...
        radar_type=radar_type)
    filename = save_path / f'{pkl_prefix}_infos_test.pkl'
    print(f'DualRadar info test file is saved to {filename}')
    mmengine.dump(kitti_infos_test, filename)
